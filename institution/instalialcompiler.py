...
class InstalCompiler(object):
    ...
    def instal_print_noninertials(self, whens: list) -> None:
        ...
        for rl in whens:
            ...
            print("what is passed to the function "+ante)
    ...
    def printCondition(self, c, institution: str=None):
        ...
        if c[0] == 'and':
            ...
            #Do not include revise in the output
            #if c[1] != "revise":
            #self.instal_print("  SPECIAL Insert {fluent}".format(fluent=c))
...
### Updated Jul 14-18,2021
    ...
    def print_prep_noninertial(self, whens: list) -> None:
        ...
        for rl in whens:
            ...
            if check:
                ...
                revise = self.checkRevisable(ante,"revise")

                if revise:
                    #Print with try and use 
                    bId = 1
                    self.bId=1
                    #bId = 0
    ...
    def print_prep_generates(self, generates: list) -> None:
        ...
        for rl in generates:
            ...
            revise = self.checkRevisable(cond,"revise")
            ...
            if revise:
                ...
                for x in inits:
                    ...
                    self.instal_print(
                        "%\n% Translation of {inev} initiates {inits} if {condition}"
                        "\n% Rule ID: {rId}"
                        .format(inev=self.extendedterm2string(inev), inits=x, condition=cond,rId=self.rId))
                    self.instal_print("%\ninitiated({inf},{inst},{ev},I) :-\n" # 2019604 JAP: added ev
                                    "   occurred({ev},{inst},I),\n"
                                    "   not occurred(viol({ev}),{inst},I),\n"
                                    "   holdsat(live({inst}),{inst},I)," # 20190606 JAP: add arg to holdsat
            ...
            if revise:
                ...
                for x in terms:
                    ...
                    self.instal_print(
                        "%\n% Translation of {inev} terminates {terms} if {condition}"
                        "\n% Rule ID: {rId}"
                        .format(inev=self.extendedterm2string(inev), terms=x, condition=cond,rId=self.rId))
                    self.instal_print("%\nterminated({inf},{inst},{ev},I) :-\n" # 2019604 JAP: added ev
                                    "   occurred({ev},{inst},I),\n"
                                    "   not occurred(viol({ev}),{inst},I),\n"
                                    "   holdsat(live({inst}),{inst},I)," # 20190606 JAP: add arg to holdsat
    ...
    def prepPrintException(self,rId, h):
        ...
        temp=temp.replace("L","+location",1)
        temp=temp.replace("P","+person",1)
        temp=temp.replace("N","+number",1)
        temp=temp.replace(",rooms,",",$inst,")
        # use 2 on the parameters as an XHAIL hack 
        temp=temp.replace("L","+location2")
        temp=temp.replace("P","+person2")
        temp=temp.replace("N","+number2")
