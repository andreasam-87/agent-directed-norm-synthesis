...
class InstalCompiler(object):
    ...
    def compile_ial(self, ial_ir: dict) -> str:
        ...
        print(self.ial_ir,file=open('outDict.txt','w'))
    ...
    #reworked for the clingo 5 version though had a version already 
    def printAggregates(self,c,term,institution: str=None):
        ...
        if l==1:
            self.instal_print("holdsat({nif},{inst},nif,I) :- {var} = #count {{ P: " 
            ...
            self.instal_print(" holdsat({fluent},{inst},_,I) }}," # 20190606 JAP: add arg to holdsat
                            .format(fluent=self.extendedterm2string(c[0]), inst=institution))
            vars1 = {}
            vars1_temp = {}
            ...
            for item in c:
                if(ct==l):
                    self.instal_print(" holdsat({fluent},{inst},_,I) }}," # 20190606 JAP: add arg to holdsat
                            .format(fluent=self.extendedterm2string(item), inst=institution))
           
                    vars1 = {}
                    vars1_temp = {}
                    self.collectVars(item, vars1_temp)
            
                    for key, value in vars1_temp.items():
                        if key not in vars_head:
                            vars1[key]=value

                    #self.instal_print("ET size - "+str(len(extra_types)))
                    for key, value in extra_types.items():
                        if key not in vars1:
                            vars1[key]=value
                        #self.instal_print("ET - "+key+" - "+value)
                    ...
                    #    self.instal_print("   {tvar},".format(ptvar=k))
                    ...
                    for k in vars1:
                        #etypes.add(k)
                        extra_types.update({k:self.types[vars1[k]]})
                        #self.instal_print("   {pred}({tvar}),".
                                #format(pred=self.types[vars1[k]], tvar=k))
                ct+=1
