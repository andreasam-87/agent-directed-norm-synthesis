...
class InstalCompiler(object):
    ...
    def instal_print_noninertials(self, whens: list) -> None:
        ...
        for rl in whens:
            ...
            # aggr = False
            # for k in vars1:
            #     pred=self.types[vars1[k]]
            #     print('Str: ',pred, ' k: ',k)
            #     if str is 'aggrCount': 
            #         print('found aggrCount')
            #         aggr = True
            #         break
            #     else:
            #         self.instal_print("   {pred}({tvar}),".
            ...
            #     for k in vars1:
            #         if c==0: 
            #             self.instal_print(" {tvar} = #count( {tvar}: ".
            #                     format( tvar=k))
            #             c+=1
            #         else: 
    ...
    def printAggregates(self,c,term,institution: str=None):
        ...
        str_aggr_print = "holdsat("+self.extendedterm2string(self.temp)+","+institution+",nif,I) :- "+ t +" =  #count {holdsat("+self.temp[0]+str(ct)+"(vars),"+institution+",nif,I) : types} "
       
        #self.instal_print(" {} = #count {{".format(t) )


        #str_new_fluent ="%%%"+str(c[0][0])+str(ct)+"(vars) :- "
        str_new_fluent ="holdsat("+self.temp[0]+str(ct)+"(vars),"+institution+",nif,I) :- "
        ...
        if l==1:
            self.instal_print("holdsat({nif},{inst},nif,I) :- {var} = #count {{" 
                              .format(nif=self.extendedterm2string(self.temp),inst=institution,var=t))
             #self.instal_print(" {} = #count {{".format(t))
            
            self.instal_print(" holdsat({fluent},{inst},E{count},I) : event(E{count}) :" # 20190606 JAP: add arg to holdsat
                            .format(fluent=self.extendedterm2string(c[0]), inst=institution,count=ct))
            vars1 = {}
            vars1_temp = {}
            self.collectVars(c[0], vars1_temp)
           # print("WWWWWWWWWW",type(vars1), " ", vars1)
            #print("QQQQQ resulting list: "+str(list(set(vars1).symmetric_difference(set(vars_head))))) 
           # print("QQQQQ resulting list: ",list(set(vars1).difference(set(vars_head))))
            #vars1=list(set(vars1).difference(set(vars_head))) #for unique items in lists
            #vars1 = dict(vars1)
            #print("WWWWWWWWWW",type(vars1), " ", vars1)
            ...
            for item in c:
                if(ct==l):
                    ...
                    #self.instal_print(" holdsat({fluent},{inst},E{count},I) : {add} :" # 20190606 JAP: add arg to holdsat
                     #           .format(fluent=self.extendedterm2string(item), inst=institution,count=ct,add=str_event))
                    # self.instal_print(" holdsat({fluent},{inst},E{count},I) : {add} }}," # 20190606 JAP: add arg to holdsat
                    #            .format(fluent=self.extendedterm2string(item), inst=institution,count=ct,add=str_event))
                    str_new_fluent+="holdsat("+self.extendedterm2string(item)+","+institution+",E"+str(ct)+",I),"
            ...
            #str_vars ="" 
            #ct2=1
            #l2=len(str_new_fluent_vars)   
            #for s in str_new_fluent_vars: 
            #    if ct2==l2:
            #        str_vars+=s 
            #    else:
            #        str_vars+=s+"," 
            ...
            #Get a unique list of types for the aggregates
            #str_vars ="" 
            #ct2=1
            #l2=len(set_new_types)   
            #for s in set_new_types: 
            #    if ct2==l2:
            #        str_vars+=s 
            #    else:
            #        str_vars+=s+" : " 
    ...
    def printCondition(self, c, institution: str=None):
        ...
        if not c:
            return
        if c[0] == 'and':
            #Modified by Andreasa 23/06/2021 - BEGINS
            #MOVED TO PRINT NON-INERTIALS FUNCTION - 07/07/2021
            #print('C- ',c)
            
            #check,ar,trm = self.checkAggregates(c)
            #if check:
            #      self.printAggregates(ar,trm,institution=institution)
            #      return
...
#def instal_print_noninertials(self, whens: list) -> None:
    ...
    def print_prep_noninertial(self, whens: list) -> None:
        ...
        #learning_print("%")
        #learning_print("% ---------------------------------------------------------------- ")
        #learning_print("%  Pre-processing: noninertial rules")
        #learning_print("% ---------------------------------------------------------------- ")

        #learning_print("%")
        #learning_print("% noninertial rules")
        ...
        for rl in whens:
            ...
            if not check:
                ...
                self.bId=1
                self.prepPrintTryUseForCondition(self.rId, bId, ante)
    ...
    def prepPrintExtension(self,rId, h):
        ...
        self.instal_print("   extension({rId},{head}),".format(rId = rId, head = h))
    ...
    def prepPrintTryUseForCondition(self,rId, bId, c):
        ...
        if c[0]=='and':
            ...
            self.prepPrintTryDel(rId, bId, lit, c)
            self.bId+=1
    ...
    def prepPrintTryDel(self,rId, bId, lit, ev):
        ...
        self.instal_print("try({rId}, {bId}, {lit}) :- not del({rId}, {bId}), {lit}.\n".format(rId=rId, bId=self.bId, lit=lit))
        #%try(1,1,holdsAt(inroom(A,R1),T)) :- not del(1,1), holdsAt(inroom(A,R2),T).

        #%try(1,1,holdsAt(inroom(A,R1),T)) :- del(1,1).
